...
public class MuteReasonInventory implements InventoryProvider {
	...
	private final ClickableItem INFO = ClickableItem
			.of(ItemBuilder.of(Material.PLAYER_HEAD).name(__.PREFIX + ChatColor.GRAY + "§b§lINFO").lore(Arrays.asList("§6You can customise ban reason by adding reason to your command")).texture("eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvZDAxYWZlOTczYzU0ODJmZGM3MWU2YWExMDY5ODgzM2M3OWM0MzdmMjEzMDhlYTlhMWEwOTU3NDZlYzI3NGEwZiJ9fX0=").build());	@Override
	public String title(Inventory inv) {
		// TODO Auto-generated method stub
		return "§cChoose ban reason";
	}
	...
	public void update(Inventory inv) {
		...
		List<String> messages = Terminator.get().getConfigManager().getMuteReason();
		int mutePages = (messages.size() - 1) / 18 + 1;
		...
		for (int i = 0; i < 18; i++) {
			...
			inv.set(i + 9, ClickableItem.of(getItemMuteReason(reason	) , e->{
				e.getWhoClicked().closeInventory();
				Mute mute = (Mute) inv.get(MUTE);
				mute.setReason(reason);
				Terminator.get().getTerminatorManager().mute(mute.getPunishedUuid(), mute.getPunisherUuid(), mute.getReason(), mute.getExpiration());
			}));
		}
	}

	private ItemStack getItemMuteReason(String msg){
