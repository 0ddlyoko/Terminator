...
public class BanReasonInventory implements InventoryProvider {
	...
	public void update(Inventory inv) {
		...
		List<String> messages = Terminator.get().getConfigManager().getBanReason();
		int banPages = (messages.size() - 1) / 18 + 1;
		...
		for (int i = 0; i < 18; i++) {
			...
			inv.set(i + 9, ClickableItem.of(getItemBanReason(reason	) , e->{
				e.getWhoClicked().closeInventory();
				Ban ban = (Ban) inv.get(BAN);
				ban.setReason(reason);
				Terminator.get().getTerminatorManager().ban(ban.getPunishedUuid(), ban.getPunisherUuid(), ban.getReason(), ban.getExpiration());
			}));
		}
	}

	private ItemStack getItemBanReason(String msg){
