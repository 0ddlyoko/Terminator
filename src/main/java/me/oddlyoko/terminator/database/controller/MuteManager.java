...
public class MuteManager {
	...
	public long addMute(MuteModel muteModel, DatabaseModel model) throws SQLException {
		...
		try {
			...
			s.setString(1, muteModel.getPunishedUuid().toString());
			s.setString(2, muteModel.getPunisherUuid() == null ? null : muteModel.getPunisherUuid().toString());
	...
	public int stopMute(MuteModel muteModel, DatabaseModel model) throws SQLException {
		...
		try {
			...
			s.setString(2, muteModel.getDeletePlayer() == null ? null : muteModel.getDeletePlayer().toString());
	 ...
	 *                          If error
	 */
	public List<UUID> getPlayersUUIDMutedOnce(DatabaseModel model) throws SQLException {
		Connection c = null;
		PreparedStatement s = null;
		ResultSet rs = null;
		try {
			...
			Bukkit.getLogger().log(Level.SEVERE, "getPlayersUUIDMutedOnce: SQLException while retrieving mutes", ex);
			throw ex;
		} finally {
			close(c, s, rs);
		}
