...
import me.oddlyoko.terminator.mutes.Mute;

public class MuteHistInventorry implements InventoryProvider{
	...
	public void init(Inventory inv) {
		List<Mute> mutes = Terminator.get().getMuteManager().getSortedMuteHistory();
		int page = (int) inv.get(PAGE);
		NEXT = ClickableItem.of(INEXT , e ->{
			int pagee = (int) inv.get(PAGE);
			inv.put(PAGE, pagee+1);
			loadHeads(inv, mutes, (int)inv.get(PAGE));
			ChangeButtons(inv);
		});
		...
		Double d = (double) (mutes.size()/27);
		maxPage = d.intValue()+1;
		loadHeads(inv, mutes, (int)page);
		ChangeButtons(inv);
	}
	...
	private ItemStack getBanSkull(UUID uuid , Mute mute) {
		//Nom du joueur
		//Banni par 
		//le 
		//Jusqu'au
		...
		meta.setDisplayName("§b"+Bukkit.getOfflinePlayer(mute.getMutedUuid()).getName());
		List<String> lore = new ArrayList<>();
		lore.add("§eMute par §f"+Bukkit.getOfflinePlayer(mute.getMuterUuid()).getName());
		lore.add("§eLe §f" + format.format(mute.getCreationDate()));
		lore.add("§eJusqu'au §f"+format.format(mute.getExpiration()));
		lore.add("§ePour §f" + mute.getReason());
		String status = mute.isDeleted() ? "§a§l[Expiré]" : "§c§l[Active]";lore.add(status);
		meta.setLore(lore);
	...
	public void loadHeads(Inventory inv , List<Mute> mutes	 , int page) {
		inv.fillRectangle(9, 9, 3, ClickableItem.of(ItemBuilder.of(Material.AIR).build()));
		int checkout = mutes.size()<=27 ? mutes.size() : 27*page;
		for(int i = page*27-27 ; i<checkout ; i++) {
			Mute mute =mutes.get(i);
			int pos = i-((page-1)*27)+9;
			inv.set(pos, ClickableItem.of(getBanSkull(mute.getMutedUuid(), mute)));
			if(i==mutes.size()-1) {
				}
			}
	}
